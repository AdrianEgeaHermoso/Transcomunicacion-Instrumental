...
public class Tci {
  public static void main(String[] args) {
    ...
    try {
      ...
      int t = rutaPortadora.length(); // Aquí se recoge el tamaño del argumento
      String rutaPortadoraConExtension = rutaPortadora.substring(0, t - 4); // Aquí se recoge todo el nombre
      int tamanoPalabra = 0;

      if (rutaPortadora.charAt(t - 4) == '.') {
        rutaPortadoraModificada = rutaPortadoraConExtension + ".html"; // Formato final del nombre si tiene extension

      } else {
        rutaPortadoraModificada = rutaPortadora + ".html"; // Formato final del nombre si no tiene extension
      ...
      int nParrafos = 0;
      String linea;
      System.out.println("--Generando el archivo de la señal portadora del mensaje--");
      while (nParrafos > 40 || nParrafos < 1) {

        System.out
            .print("Introduzca el numero de parrafos que desea recoger con la portadora (Minimo: 1 - Maximo: 40): ");
        nParrafos = s.nextInt();
      ...
      } // Fin de bucle while para recoger el numero de parrafos

      while (tamanoPalabra < 3) {

        System.out.print("Introduzca el tamaño minimo para filtrar las palabras (debe ser mayor que 2): ");
        tamanoPalabra = s.nextInt();

        if (tamanoPalabra < 3) {
          System.out.println("ERROR. El valor introducido no es valido, "
              + "por favor vuelva a intentarlo.\n");
        }
      } // Fin de bucle while para recoger el tamaño minimo de palabras

      generaPortadora(rutaPortadora, nParrafos); // Genera el archivo portador con el mensaje

      generaCabecera(bw); // Genera la cabecera para procesar el archivo de la portadora

      brPortadora = new BufferedReader(new FileReader(rutaPortadora));
      linea = brPortadora.readLine();

      System.out.println("--Procesando el archivo con la señal de la portadora--");
      System.out.println("Analizando portadora...");

      while (linea != null) {

        generaPoratadoraModificada(rutaDiccionario, bw, linea, tamanoPalabra); // Procesa todo el mensaje de la
                                                                               // portadora con el diccionario y resalta
  ...
  private static void generaPortadora(String rutaEscritura, int tamano) throws IOException {
    ...
    while (tamano != 0) {
      numeroAleatorio = (int) (Math.random() * 140); // CONTROLA LA PROPORCION, CUANTO MAYOR SEA DE 100 MAS SIMBOLOS
                                                     // GENERA.
  ...
  private static void generaPoratadoraModificada(String rutaLectura, BufferedWriter bw, String linea, int tamanoPalabra)
